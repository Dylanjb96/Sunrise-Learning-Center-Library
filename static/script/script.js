    ...
    $.get('/books', {'query': query, 'field': field}, function(data) {
        ...
        if (query === '') {
            ...
            if (Array.isArray(data)) {
                ...
                // Assume data is a single object and handle it
                let row = $('<tr></tr>');
                row.append($('<td></td>').text(data.id));
                row.append($('<td></td>').text(data.title));
                row.append($('<td></td>').text(data.author));
                row.append($('<td></td>').text(data.genre));
                row.append($('<td></td>').text(data.year));
                row.append($('<td></td>').text(data.stock));
                row.append($('<td></td>').text(data.available));
                table.append(row);
            }
        }
    });
            ...
            success: function(data) {
                ...
                books.forEach(function(book) {
                    transactions.forEach(function(transaction) {
                        if (memId == transaction.borrower_id && transaction.type == 'borrow' && book.id == transaction.book_id) {
                            ...
                            row.append($('<td></td>').text(book.author));
                            row.append($('<td></td>').text(book.genre));
                            row.append($('<td></td>').text(book.published));
                            if (date <= currentDate) {
                            ...
                            let inputBook = $('<input>').attr("name", 'id').attr('value', book.id).hide(); 
                            ...
                            $('#returnAll').append(($('<input>')).attr('class','inputAll').attr('name', 'all_ids').attr('value', book.id).hide());
                        }
                    });
                });
